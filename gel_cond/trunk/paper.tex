...
In Figure~\ref{fig:breaking}d we show the time evolution of the probability for a topologically relevant strand rupture $P_\text{break}(L>3)$, i.e. the ratio of the number of bond breaking events with $L>3$ by the number of bonds. Interestingly, for all samples that show percolation before coarse graining, see Figure~\ref{fig:phasediag}b, strand rupture is maximum just after percolation. 



We saw in Figure~\ref{fig:phasediag}c that at these times, the strands are in average only one or two particles thick and thus easily break under tension.
