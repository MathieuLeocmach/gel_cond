...
\begin{figure*}
	\includegraphics{figs/cell_vs_cap.pdf}
...
In Figure~\ref{fig:cell_vs_cap}d we show the phase diagram obtained by our protocol. We observe three possible behaviours: at low polymer concentration ($c_p<\SI{0.2}{\gram\per\litre}$) sample fully relax to a fluid state; at very low colloid volume fraction ($\phi<0.05$) and high polymer concentration the particles condense into long-lived well separated clusters as observed in~\cite{Lu2006}; in the rest of the explored phase space we observe a long-lived network. We rationalise these observations by superimposing the spinodal line obtained by generalised free volume theory~\cite{Fleer2008}. 

XXX discuss differences between theory and experiment. What is cluster phase? XXX

\begin{figure}
\includegraphics{figs/wholeprocess.pdf}
\caption{\textbf{Gelation process} \textbf{a} Reconstruction of experimental coordinates coloured by the number of particles in clusters in a typical sample close to the cluster-gel line ($\phi=7.5~\%$, $c_p=\SI{1}{\gram\per\litre}$). Origin of time is the last frame before melting of Wigner crystal. \textbf{b} Comparision of system evolution in terms of largest cluster extent and of mean coordination number. By increasing density: $\phi=4.2,\,8,\,16,\,27~\%$, $c_p=1,\,1.5,\,1.2,\,\SI{1}{\gram\per\litre}$ for \textcolor{red!40!yellow}{$*$}, \textcolor{red!80!yellow}{$\circ$}, \textcolor{red!80!black}{\tiny$\blacksquare$} and \textcolor{red!40!black}{$\bullet$} respectively.\textbf{c} Growth of the characteristic wave number for the same samples.}
\label{fig:wholeprocess}
\end{figure}


Figure~\ref{fig:wholeprocess}a shows a computer reconstruction from experimental coordinates of a typical gelation experiment close to the cluster-gel boundary. Immediately before ions enter the cell ($t=0$), the suspension is in a Wigner crystal state where the particles are far apart due to long range repulsion. As soon as the charges are screened, the particles begin to aggregate and form clusters that progressively connect to each other while coarsening to finally percolate through the field of view at $t=\SI{35}{\minute}$.

However, this sequence of events is not universal, as shown in Figure~\ref{fig:wholeprocess}b. At higher densities, percolation happen first, within the first few $\tau_B$ after charge screening and then coarsening proceed with an increasing average coordination number $\bar{N}_C$. At lower densities, percolation never takes place and we observe the compaction of individual clusters.

Universality is recovered when investigating the dynamics of the coarsening process. We compute the characteristic wave number $\langle q \rangle$ of the structure factor $S(q)$ and show its temporal evolution in Figure~\ref{fig:wholeprocess}c. We confirm the appearance of a low $q$ peak in $S(q)$, see Supplementary Figure XXX, which is characteristic of spinodal decomposition in a system of a conserved order parameter. At short times $\langle q \rangle$ shows a plateau indicating that the low $q$ peak builds up at constant wave number. At intermediate times, $\langle q \rangle$ scales as $t^{-1/3}$, again coherent with spinodal decomposition kinetics. Finally at longer times each sample deviates from the universal slope to form a plateau indicating arrest. The more dilute samples arrest sooner.
