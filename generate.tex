...
\begin{figure}
	...
	\begin{tikzpicture}[lab/.style={below right, text height=0.8em, text depth=0.2em, font=\Large\bfseries}]
		\begin{groupplot}[%
			...
			\nextgroupplot[
			height=0.8\columnwidth-1em,
			xlabel={$t/\tau_B$}, xmin=0, xmax=20,%
			ylabel={$R_g/\sigma$},
			]%
			\begin{scope}[every node/.style={inner sep=0.4em}]
				\node[circle, ball color=gray] at (axis cs:2.5,1.6) (n3_01){};
				\node[circle, ball color=gray, right=0 of n3_01] (n3_02){};
				\node[circle, ball color=gray, above right= -0.4em and +0.4em of n3_02] (n3_03){};
				\node[circle, ball color=gray] at (axis cs:4,1.4) (n3_11){};
				\node[circle, ball color=gray, below right=-0.4em and 0.4em of n3_11] (n3_12){};
				\node[circle, ball color=gray, above right= +0 and -0 of n3_12] (n3_13){};
				\node[circle, ball color=gray] at (axis cs:18,1.35) (n3_21){};
				\node[circle, ball color=gray, above right=0.3em and -0.4em of n3_21] (n3_22){};
				\node[circle, ball color=gray, right=0 of n3_21] (n3_23){};
			\end{scope}
			\addplot table [x expr=\thisrowno{0}*6, y expr=\thisrowno{1}]{cluster_Rg_dynamics_N3.csv};
			\addplot table [x expr=\thisrowno{0}*6, y expr=\thisrowno{2}]{cluster_Rg_dynamics_N3.csv};
		...
		\node[lab] at (g c1r2.outer north west) {b};
	\end{tikzpicture}
...
\begin{figure*}
    ...
    {rand^2};
		...
		\node[lab] at (phasediag.outer north west) {b};
	\end{tikzpicture}
	\caption{\textbf{Reservoir cell} \textbf{a} Sketch of our experimental setup. The observation cell contains initially colloids, polymer and no salt. \textbf{b} Phase diagram. \textbf{c-e} Comparison between gels made at the same state point ($\phi=27~\%$, $c_p=\SI{0.75}{\gram\per\litre}$) either in situ (\textbf{c}, blue) or ex situ (\textbf{d}, red).}
...
\begin{figure*}
	...
	group style={
		...
		width=0.4\textwidth,%
		height=0.2\textwidth+1em,%
		ylabel absolute, every axis y label/.append style={anchor=base, yshift=-1.5em}
		]
		\nextgroupplot[
			xlabel={time (min)},
			ymin=0, ymax=3, ylabel={Porod exponent},]
		\addplot[no marks, error bars/.cd, y dir=both,y explicit] table[y error index=2] {155C.porod};
		
		\nextgroupplot[
			xmode=log, ymode=log,
			xlabel={$(t-t_0)/\tau_B$},
			ylabel=$\langle q\rangle\sigma$,
			ytick={0.6,0.8,1,1.6,2}, yticklabels={0.6, 0.8,1,1.6, 2},
			]
		\addplot[only marks] table[x expr=6*(\thisrowno{0}-5)]{155C.qmax};
		\addplot+[red, no marks, domain=50:300] {6*x^(-1/3)} node[midway, above]{$-1/3$};
	\end{groupplot}
	
	
	\matrix[
		matrix of nodes, inner sep=0, column sep=0.0125\textwidth, row sep=0.5em,
		matrix anchor=north west,
		at={($(g c1r1.outer north east)+(-\textwidth,0)$)},
		] (m){
		\SI{5}{\minute} & \SI{10}{\minute} & \SI{20}{\minute} \\
		\includegraphics[width=0.19\textwidth]{155C_percolation_1645_p2size_t024s.png}&
		\includegraphics[width=0.19\textwidth]{155C_percolation_1645_p2size_t044s.png}&
		\includegraphics[width=0.19\textwidth]{155C_percolation_1645_p2size_t084s.png}\\
		 \SI{30}{\minute} & \SI{40}{\minute} & \SI{50}{\minute}\\
		\includegraphics[width=0.19\textwidth]{155C_1715_ageing_p2size_t00s.png}&
		\includegraphics[width=0.19\textwidth]{155C_1715_ageing_p2size_t20s.png}&
		\includegraphics[width=0.19\textwidth]{155C_1715_ageing_p2size_t40s.png}\\
		};
	
	\end{tikzpicture}
\caption{\textbf{Gelation observed in-situ} in a typical sample ($\phi=7.5~\%$, $c_p=\SI{1}{\gram\per\litre}$). Origin of time is salt introduction. \textbf{a} Reconstruction of experimental coordinates coloured by the number of particles in clusters. \textbf{b} Evolution of Porod exponent. \textbf{c} Growth of the characteristic wave number. Time origin is shifted to correspond to the melting of the Wigner crystal.}
